/// @description uiz_getDivisionMiddles_margins(space, divisions, square, margin)
/// @param space
/// @param  divisions
/// @param  square
/// @param  margin
function uiz_getDivisionMiddles_margins(argument0, argument1, argument2, argument3) {
	//if you imagine a 1D space that needs to be filled with a certain amount of squares(argument1,divisions)
	//then this function will return the middle for each square.
	//it will however add margins to the left and right side (a margin of 0.2 would mean 0.2 margin on the left and 0.2 on the right)
	//see manual for picture.
	return (argument2+0.5)*(argument0-argument3*2)/argument1+argument3



}
