...
function uiz_gridlist_updateoldcell() {
	...
	//sdbm("updating mark",selectedx_old,cur_wat_old+rx,cur_hat_old+yh,cur_nwat_old+rx,cur_nhat_old+yh)
	            ...
	            //    uiz_updater_FixViews_area_selfmarked(cur_wat_old+rx,max(yh,cur_hat_old+yh),cur_nwat_old+rx-1,min(rly,cur_nhat_old+yh+1));//the new view
	            //}else{
	                uiz_updater_FixViews_area_selfmarked(cur_wat_old+rx,max(yh,cur_hat_old+yh),cur_nwat_old+rx,min(rly,cur_nhat_old+yh+1));//the new view
	            //}
	        break;
	        case 2:
	            uiz_updater_FixViews_area_selfmarked(rx,max(yh,cur_hat_old+yh),rlx-scrollbarw,min(rly,cur_nhat_old+yh+1));//the new view
	        break;
	        case 3:
	            //if selectedx_old < ds_list_size(mainlist)-1 then{
