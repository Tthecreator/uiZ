function uiz_textarea_moveCursorLeft() {
	...
	if selection1Char==0 then{//move one line up
	    ...
	    selection1X = ix + string_width(string_hash_to_newline(string_copy(line,1,selection1Char)));
	    }
	}else{//move on the same line
	    selection1X-=string_width(string_hash_to_newline(string_char_at(textList[| selection1Line],selection1Char)));
	    --selection1Char;
	}
