...
function uiz_functionbar_drawitem(argument0) {
	var i=argument0;
	...
	//                draw_square(p, ry + marginy +30,p + isz, rly - marginy, col, 1)            
	            ...
	            draw_text(p + isz / 2, ry + marry + 3, string_hash_to_newline(uiz_getText_contained(text[i], isz)))
	            ...
	            if is_string(menu[i]) then {
	                ...
	                draw_set_halign(fa_center)
	                draw_text(p + isz / 2, rly - marry - 3, string_hash_to_newline(uiz_getText_contained(menu[i], isz)))
	            } else {
	                if menu[i] > 0 then {
