function uiz_fixmousemenupos(argument0) {
	var t=argument0;
	with(t){
	...
	if posinframex=uiz_static then{
	    ...
	    width=1;
	    for(var i=0;i<sz;i++){
	        width=max(string_width(string_hash_to_newline(string(uselist[| i]))),width);
	    }
	...
	if posinframey=uiz_static then{
	    ...
	    for(var i=0;i<sz;i++){
	        height+=margin+string_height(string_hash_to_newline(string(uselist[| i])));
	    }
