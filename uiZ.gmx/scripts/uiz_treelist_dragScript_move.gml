...
if instance_exists(argument0) and draggingItem!=-1 and itemDestination!=-1{
    with(argument0){
        if itemDestinationYPosition<0.5 then{//if we are hovering over the top of the tiem
            ...
            uiz_treelist_moveEntry_recursive(argument0,draggingItem,itemDestination+1,uiz_treelist_item_get_indentLevel(argument0,itemDestination));//add item below the destination, with the same indent as the destination
        }
        /*
