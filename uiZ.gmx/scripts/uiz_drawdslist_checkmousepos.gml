...
//check if selected
var pit=point_in_rectangle(uiz_getmouse_x(),uiz_getmouse_y(),sx,sy+addy+h,slx-scrollwidth,sy+h+addy+strh);
osel=sel;
var upd=false;
if pit {
sel=i;
if osel!=sel then{
//update
uiz_drawdslist_updateviews(true,sx,sy+addy+h,slx-scrollwidth,sy+h+addy+strh,osel,sel)
break;
}
...
//update
uiz_drawdslist_updateviews(true,sx,sy+addy+h,slx-scrollwidth,sy+h+addy+strh,oselected,selected)
break;
}
}else{
if keepselection=false and selected=i then{
//update
uiz_drawdslist_updateviews(false,sx,sy+addy+h,slx-scrollwidth,sy+h+addy+strh,oselected,selected)
selected=-1;
keepselected=-1;
break;
}
